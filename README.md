...
2. 日常运行：双击 `启动语音助手.bat`。
3. 运行环境检查：双击 `运行环境检查.bat`，或执行 `deployment_check.py`。
4. 浏览器访问本地操作台，进入工程师诊断中心可查看诊断日志。
...
- `web/assets/licenses/` 中包含 JetBrains Mono 和 Lucide 的第三方许可证文本。
- 本项目源代码许可证尚未声明。请由项目权利人选择并添加 `LICENSE` 文件后再公开发布。
